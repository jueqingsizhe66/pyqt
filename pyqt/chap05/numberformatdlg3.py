...
class NumberFormatDlg(QDialog):
    ...
    def __init__(self, format, callback, parent=None):
        ...
        punctuationRe = QRegExp(r"[ ,;:.]")# QRegExp
        thousandsLabel = QLabel("&Thousands separator")
        self.thousandsEdit = QLineEdit(format["thousandsseparator"])
        thousandsLabel.setBuddy(self.thousandsEdit)
        self.thousandsEdit.setMaxLength(1)#only QRegExp can be valid, other no
        self.thousandsEdit.setValidator( #? dizzy? --->QLineEdit
                QRegExpValidator(punctuationRe, self))#It maybe have a for loop
        ...
        self.decimalMarkerEdit.setInputMask("X") #can be replaced by other letter

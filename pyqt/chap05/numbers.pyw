...
class Form(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        layout.addLayout(buttonLayout)#addLayout and addWidget
    ...
    def refreshTable(self):
        ...
        for x in range(self.X_MAX):
            for y in range(self.Y_MAX):
                ...
                digits = []
                for i, digit in enumerate(reversed(whole)):# the last time
                    #process the string  also use (enumerate(reverse(string)))
                    if i and i % 3 == 0:
                        digits.insert(0, self.format["thousandsseparator"])
                    digits.insert(0, digit)#if not satisfy if, just insert digit
                ...
                item = QTableWidgetItem(text)# adapt to the QTable
